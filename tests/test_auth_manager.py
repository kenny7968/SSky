...
class TestAuthCredentialManager(unittest.TestCase):
    """認証情報管理クラスのテストクラス"""
    ...
    def setUp(self):
        ...
        # AuthCredentialManagerのシングルトンインスタンスを取得
        self.credential_manager = AuthCredentialManager()
        # モックのDataStoreを注入
        self.credential_manager.data_store = self.mock_data_store
    
    @patch('core.auth.credential_manager.encrypt_data')
    def test_save_encrypted_session(self, mock_encrypt):
        """save_encrypted_sessionのテスト"""
        # モックの戻り値を設定
        mock_encrypt.return_value = b'encrypted_data'
        self.mock_data_store.save_encrypted_session.return_value = True
        
        # テスト実行
        user_did = 'did:plc:test_user'
        session_data = 'session_data'
        result = self.credential_manager.save_encrypted_session(user_did, session_data)
        
        # モックが正しく呼ばれたか確認
        mock_encrypt.assert_called_once_with(session_data)
        self.mock_data_store.save_encrypted_session.assert_called_once_with(user_did, b'encrypted_data')
        
        # 結果の確認
        self.assertTrue(result)
    
    @patch('core.auth.credential_manager.encrypt_data')
    def test_save_encrypted_session_encryption_failure(self, mock_encrypt):
        """暗号化失敗時のsave_encrypted_sessionのテスト"""
        # モックの戻り値を設定
        ...
        # モックが正しく呼ばれたか確認
        mock_encrypt.assert_called_once_with(session_data)
        self.mock_data_store.save_encrypted_session.assert_not_called()
        
        # 結果の確認
        self.assertFalse(result)
    
    @patch('core.auth.credential_manager.decrypt_data')
    def test_load_encrypted_session(self, mock_decrypt):
        """load_encrypted_sessionのテスト"""
        # モックの戻り値を設定
    ...
    @patch('core.auth.credential_manager.decrypt_data')
    def test_load_encrypted_session_no_data(self, mock_decrypt):
        """セッションデータがない場合のload_encrypted_sessionのテスト"""
        # モックの戻り値を設定
    ...
    @patch('core.auth.credential_manager.decrypt_data')
    def test_load_encrypted_session_decryption_failure(self, mock_decrypt):
        """復号化失敗時のload_encrypted_sessionのテスト"""
        # モックの戻り値を設定
        self.mock_data_store.get_latest_session.return_value = ('did:plc:test_user', b'encrypted_data')
    ...
    def test_delete_session(self):
        ...
        # テスト実行
        user_did = 'did:plc:test_user'
        result = self.credential_manager.delete_session(user_did)

...
class FollowersDialog(UserListDialog):
    ...
    def __init__(self, parent, client):
        ...
        super(FollowersDialog, self).__init__(
            parent, 
            client,
            title=i18n.get_message("dialog.followers_title").format(handle=client.profile.handle),
            size=(600, 500)
        )
    ...
    def fetch_users(self):
        ...
        try:
            ...
            # ステータスを更新
            self.update_status(self.i18n.get_message("dialog.followers_status"), len(self.list_ctrl.users))
                
            logger.info(f"フォロワー一覧を取得しました: {len(result.followers)}件")
            
        except Exception as e:
            logger.error(f"フォロワー一覧の取得に失敗しました: {str(e)}")
            wx.MessageBox(self.i18n.get_message("dialog.followers_fetch_error").format(error=str(e)), self.i18n.get_message("error.title"), wx.OK | wx.ICON_ERROR)
            self.load_more_btn.SetLabel(self.i18n.get_message("button.load_more"))
            self.load_more_btn.Enable(True)

...
def encrypt_data(data: Union[str, bytes]) -> Optional[bytes]:
    ...
    try:
        ...
        logger.debug(f"暗号化後データの内容: {encrypted_data!r}")
...
def decrypt_data(encrypted_data: bytes) -> Optional[str]:
    ...
    try:
        ...
        logger.debug(f"復号化前データの内容: {encrypted_data!r}")
